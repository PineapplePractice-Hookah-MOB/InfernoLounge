...
extension TableViewModel {
  func postTable () {
    apiManager.post(tableId: tableId, people: Int(count.first?.description ?? "0") ?? 0, comment: wishes, bookedFrom: "\(selectedDate.dateFormat())", bookedTill: "\(selectedDate.addingTimeInterval(60*60).dateFormat())")
  }
}
